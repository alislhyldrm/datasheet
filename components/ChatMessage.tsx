...
function ChatMessage({ message, multiDoc }: { message: Msg; multiDoc: boolean }) {
  ...
  const components = useMemo<Components>(
    () => ({
      ...
      pre: ({ children }) => (
        <pre className="mb-3 overflow-x-auto rounded-lg border border-border bg-surface-2 p-3 font-mono text-meta last:mb-0">
          {children}
        </pre>
      ),
      blockquote: ({ children }) => (
      ...
      th: ({ children, style }) => {
        ...
        return (
          <th
            style={style}
            className={`border border-border px-2.5 py-1.5 font-medium ${numeric ? "text-right" : "text-left"}`}
          >
            {children}
          </th>
        );
      ...
      td: ({ children, style }) => {
        ...
        return (
          <td
            style={style}
            className={`border border-border px-2.5 py-1.5 ${numeric ? "text-right font-mono" : ""}`}
          >
            {children}
          </td>
        );
  ...
  if (isUser) {
    return (
      <div className="msg-enter flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-br-sm border border-border bg-surface-2 px-3.5 py-2 text-body whitespace-pre-wrap text-ink">
          {markdown}
        </div>
      </div>
    );
